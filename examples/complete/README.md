...
1. Go into the working directory ```cd redux-firestore```
1. Fetch npm modules ```npm install```
1. In Firebase console, create new project -- I called mine _redux-firestore-complete_
   1. choose default values, nothing special
   1. add a _web_ app -- I called mine _rfc-web_
   1. copy Firebase SDK's firebaseConfig settings
1. Edit **config.js**
   1. overwrite the values in ```export const firebase { ... }``` with the values from your web app (previous step).
